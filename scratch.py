...
def rating(name, observation):
    """
    Gives team rating at specific observation
    params:
        name: str 
            name of team
        observation: int
            observation of match
    returns: int
        rating, not found error if team isn't in dataset
    """
    try:
...
def record_match(winner, loser, observation):
    ...
    team_ratings.loc[observation + 1, winner] += 1
    team_ratings.loc[observation + 1, loser] += -1
...
def estimate_win(team_1, team_2, observation):
    ...
    params:
        ...
        observation: int
            observation of match
    returns: tuple
        expected win for team 1, expected win for team 2 , winner, loser
    """
...
def leaderboard(observation):
    ...
    """
    Gives team ratings from highest
    params:
        observation: int
            observation of match
    returns: list of tuples
        sorted ratings along with team name from highest to lowest 
    """
...
def plot_chart(show_elo=True, show_bucket=False, team_list=None):
    ...
    params:
        show_elo: boolean
            plots Elo ratings across match
        show_bucket: boolean
            plots frequency of win rates       
        team_list: list of str
...
# All regions 
regions = np.loadtxt('unique_region_list.txt', usecols=(0,), dtype=str, unpack=True).tolist()
...
for region in ['LCS']:
    ...
    for year in years[years.index(start_year):years.index(end_year) + 1]:
        for quarter in quarters:
            ...
            if (end_quarter == 'Spring') and (year == end_year) and (quarter == 'Summer'):
                continue
            try:
                winners += list(np.loadtxt(f'{region}_{year}_{quarter}_wins_losses.txt', usecols=(0,1), dtype=str, unpack=True, delimiter=' | ')[0])
                losers += list(np.loadtxt(f'{region}_{year}_{quarter}_wins_losses.txt', usecols=(0,1), dtype=str, unpack=True, delimiter=' | ')[1])
                teams += list(np.loadtxt(f'{region}_{year}_{quarter}_team_names.txt', usecols=(0), dtype=str, unpack=True, delimiter=' | '))
    ...
    def split_array(access):
        ...
        params:
            access: str
                Information from split_observations trying to access
        returns: list of lists
            information about specific observations
        """
        if access == 'Year':
            k = 1
        if access == 'Quarter':
...
team_ratings_csv.to_csv('team_ratings.csv', index=False)
expected_bucket_csv.to_csv('expected_win_rates.csv', index=False)
